...
async function checkPermissions() {
    try {
        const response = await fetch('api/CSD/check_admin.php');
...
async function deleteRow(id) {
    ...
    try {
        const response = await fetch('api/CSD/update_downtime.php', {
            method: 'POST',
            headers: {
...
async function updateDowntimeTable(period = 'today', line = 'all') {
    try {
        const response = await fetch(`api/CSD/get_downtime_table.php?period=${period}&line=${line}`);
        const data = await response.json();
