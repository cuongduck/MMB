...
async function updateDowntimeChart(period) {
    try {
        ...
        const response = await fetch(`api/F2/get_downtime_chart.php?period=${period}`);
        cachedData = await response.json();
        
        if (currentView === 'totalF2') {
            renderTotalF2Chart(cachedData.totalF2, cachedData.lineData);
        } else {
...
function switchChart(view) {
    ...
    if (cachedData) {
        if (view === 'totalF2') {
            renderTotalF3Chart(cachedData.totalF2, cachedData.lineData);
        } else {
            renderLineChart(cachedData.lineData);
        }
    } else {
